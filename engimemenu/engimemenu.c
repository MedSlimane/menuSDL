...
int EnigmeMenu_Init(EnigmeMenu* menu) {
    ...
    menu->window = SDL_CreateWindow("Sous Menu Enigme", SDL_WINDOWPOS_CENTERED,
    ...
    menu->background =
        IMG_LoadTexture(menu->renderer, "assets/background4.png");
    ...
    menu->btnPuzzle =
        MakeBtn(menu->renderer, cx + 50, 400, 300, 120, "assets/btn_puzzle.png",
                "assets/btn_puzzle_h.png");

    /* View 2 – quiz answers */
    int ay = 700, aw = 150, ah = 80, agap = 200;
    int ax = cx - (3 * aw + 2 * agap) / 2;
    menu->btnA = MakeBtn(menu->renderer, ax, ay, aw, ah, "assets/btn_a.png",
                         "assets/btn_a_h.png");
...
void EnigmeMenu_Render(EnigmeMenu* menu) {
    ...
    if (menu->view == ENIGME_VIEW_CHOICE) {
        ...
        if (menu->titleFont) {
            ...
            SDL_Surface* surf =
                TTF_RenderUTF8_Blended(menu->titleFont, "QUIZ", w);
            if (surf) {
                SDL_Texture* t =
        ...
        if (menu->font && menu->question) {
            ...
            SDL_Surface* surf =
                TTF_RenderUTF8_Blended(menu->font, menu->question, blk);
            if (surf) {
                SDL_Texture* qt =
                    SDL_CreateTextureFromSurface(menu->renderer, surf);
                int tw = surf->w, th = surf->h;
...
void EnigmeMenu_Cleanup(EnigmeMenu* menu) {
    Button* all[] = {&menu->btnQuiz, &menu->btnPuzzle, &menu->btnA, &menu->btnB,
                     &menu->btnC};
    for (int i = 0; i < 5; i++) {
